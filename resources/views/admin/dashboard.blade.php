...
-------------------------- */
    ...
    {{-- CSS REVENUE CARD - ORGANIZAR DEPOIS PARA UMA PASTA CSS --}}
    <style>
        .revenue-card {
            width: 50%;
            margin: 0 auto;
        ...
        .revenue-card .metric-row>div h4 {
            margin: 0;
            font-size: 15px;
            font-weight: 700;
            color: #ffffff;
        ...
        .revenue-card .metric-row>div:nth-child(2) h4 {
        ...
        .revenue-card .trend i {
            font-size: 13px;
        }
    </style>

    {{-- CARD REVENUE METRICS --}}
    <div class="metric-card revenue-card">
        ...
        <div class="card-body">
            ...
            <div class="metric-row">
                <div>
                    <p>Hoje</p>
                    <h4>@dollar($revenueMetrics['today'])</h4>
                </div>
                <div>
                    <p>Ontem</p>
                    <h4>@dollar($revenueMetrics['yesterday'])</h4>
                </div>
                <div class="trend {{ $revenueMetrics['todayChange'] >= 0 ? 'up' : 'down' }}">
                    <i class="fas fa-arrow-{{ $revenueMetrics['todayChange'] >= 0 ? 'up' : 'down' }}"></i>
                </div>
            </div>
            ...
            <div class="metric-row">
                ...
                <div>
                    <p>Mês passado</p>
                    <h4>@dollar($revenueMetrics['lastMonth'])</h4>
                </div>
    ...
    {{-- FATURAMENTO E OUTRAS METRICAS COMECA --}}
    ...
    <div class="metrics-top-grid">
        <div class="metric-card glass-card">
            ...
            {{-- DEPOIS --}}
            <h3 class="metric-value">@dollar($totals['revenue'])</h3>
            <div class="metric-footer">
                {{-- VER SOBRE A SETA - AINDA ERRADA  --}}
                {{-- <span class="metric-change {{ $growth >= 0 ? 'positive' : 'negative' }}">
    {{ number_format($growth, 2) }}%
    <i class="fas fa-caret-{{ $growth >= 0 ? 'up' : 'down' }}"></i>
</span>
    <span class="metric-period">vs período anterior</span> --}}
            </div>
                                        ...
                                        @foreach ($aliases as $alias)
                                            ...
                                            @if ($profit > 0)
                                                <div class="bar-segment {{ strtolower($alias) }}-segment"
                                                    ...
                                                    data-value="@dollar($profit)"
                                                    data-platform="{{ ucfirst($alias) }}"
                ...
                <table class="metrics-main-table">
                    ...
                    <tbody>
                        @php
                            ...
                            $mainPlatforms = $sources->filter(
                                fn($s) => in_array(strtolower($s->alias), $soloPlatforms),
                            );
                        @endphp
                        ...
                        @foreach ($mainPlatforms as $source)
                            ...
                            <tr class="campaign-row" onclick="toggleDetails('{{ $aliasKey }}')">
                                ...
                                {{-- DEPOIS --}}
                                <td>@dollar($source->total_revenue)</td>
                        ...
                        @else row-neutral @endif">
                                                ...
                                                {{-- DEPOIS --}}
                                                <div class="acc-col">@dollar($acc->total_revenue)</div>
                        ...
                        <tr class="campaign-row" onclick="toggleDetails('{{ $nativeKey }}')">
                            ...
                            {{-- DEPOIS --}}
                            <td>@dollar($nativeGroup->sum('total_revenue'))</td>
                        ...
                        <tr id="details-{{ $nativeKey }}" class="details-row" style="display: none;">
                                ...
                                <div class="accounts-expand">
                                    ...
                                    @foreach ($nativeGroup as $sub)
                                        ...
                                        <div class="account-row row-neutral"
                                            ...
                                            <div class="acc-col acc-main"><span>{{ ucfirst($sub->alias) }}</span>
                                            </div>
                                            <div class="acc-col">@dollar($sub->total_cost)</div>
                                            {{-- DEPOIS --}}
                                            <div class="acc-col">@dollar($sub->total_revenue)</div>
                            ...
                            @else row-neutral @endif">
                                                    ...
                                                    <div class="acc-col acc-main"><span>{{ $acc->source }}</span>
                                                    </div>
                                                    <div class="acc-col">@dollar($acc->total_cost)</div>
                                                    {{-- DEPOIS --}}
                                                    <div class="acc-col">@dollar($acc->total_revenue)</div>
                                                    ...
                                                    <div
                                                        class="acc-col {{ $acc->roi >= 0 ? 'positive' : 'negative' }}">
                                                        {{ number_format($acc->roi * 100, 2, ',', '.') }}%
                                                    </div>
