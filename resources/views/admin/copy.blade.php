            ...
            <table class="metrics-main-table">
                ...
                <tbody>
                    ...
                    @foreach ($copies as $copy)
                        ...
                        {{-- linha principal --}}
                        <tr class="editor-row clickable-row" data-editor-id="{{ $copy->user_id }}" onclick="toggleDetails('{{ $key }}')">
                            ...
                            <td colspan="8" class="details-cell">
                                <div class="nested-table-container custom-scrollbar">
                                    ...
                                    @if (($creativesByAgent[$copy->user_id] ?? collect())->isEmpty())
                                        <p class="no-data-message">Nenhum criativo encontrado para este copywriter neste período.</p>
                                    @endif
                                </div>
                            </td>
                        </tr>
        ...
        <div class="subview-body-container">
            ...
            <div class="editor-chart-section glass-card">
                <div class="editor-chart-header">
                    ...
                    <div class="chart-controls">
                        <button id="toggleDailyChartTypeBtnCopy" class="btn-chart-control" title="Alternar para Linhas">
                            <i class="fas fa-chart-bar"></i>
                        </button>
            ...
            <div class="niche-data-section glass-card">
                ...
                <div id="nicheTableViewCopy" class="niche-content-view active custom-scrollbar" style="max-height: 400px; overflow-y: auto; margin-top: 15px;">
                    <table id="detailsNicheTableCopy" class="nested-table details-subview-table">
                        <thead>
                            <tr>
                                ...
                                <th data-sort-key="total_copies" class="sortable details-sortable">Copies <i class="fas fa-sort"></i></th>
                    ...
                    <p id="nicheNoDataMessageCopy" class="no-data-message" style="display: none; text-align: center;">Nenhum dado de nicho encontrado.</p>
                </div>
...
<script>
        ...
        if (row.style.display === "none") {
            ...
            document.querySelectorAll('.details-row').forEach(r => {
                if (r.id !== `details-${key}` && r.style.display !== "none") {
                    ...
                    const otherCopyRow = document.querySelector(`tr.clickable-row[data-editor-id="${otherKey.replace('copywriter-', '')}"]`);
                    if(otherCopyRow) {
    ...
    function switchNicheViewCopy(viewType) {
        ...
        document.getElementById(`niche${viewType.charAt(0).toUpperCase() + viewType.slice(1)}ViewCopy`).style.display = 'block';
        document.getElementById(`niche${viewType.charAt(0).toUpperCase() + viewType.slice(1)}ViewCopy`).classList.add('active');
    ...
    function fetchChartDataCopy(creativeCode) {
         document.getElementById("chartCreativeTitleCopy").innerText = `Carregando dados para ${creativeCode}...`;
         // SIMULACAO DE DADOS
         return new Promise(resolve => {
             setTimeout(() => {
                 document.getElementById("chartCreativeTitleCopy").innerText = creativeCode;
                 const mockData = [
    ...
    async function openCreativeChart(creativeCode, creativeTitle) {
        ...
        if (data.length > 0) {
            ...
            document.getElementById("chartCreativeTitleCopy").innerText = `${creativeTitle} (Sem dados no período ainda)`;
        }
    }
